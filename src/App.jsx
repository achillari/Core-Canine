...
win.document.write(`<html><head><title>${tmpl?.name} Roster</title><style>body{font-family:Georgia,serif;padding:40px;max-width:720px;margin:auto}h1{border-bottom:3px solid #C9933A;padding-bottom:12px}.client{margin-bottom:20px;padding:14px;border:1px solid #E8E4DC;border-radius:8px}@media print{button{display:none}}</style></head><body> <h1>🐾 ${tmpl?.name}</h1> <p>Starts ${fmtDate(inst.startDate)} · ${fmt12(inst.time)} · Instructor: ${instructor?.name}</p> <p><b>All Dates:</b> ${sessionDates.map(d => fmtDate(d)).join(" · ")}</p> <b>Enrolled (${enrolled.length}/${tmpl?.maxDogs}):</b> ${enrolled.map((c, i) => `<div class="client"><b>${i + 1}. ${c.name}</b><br><small>${c.email} · ${c.phone}</small>${c.dogs.map(d => `<br>🐕 ${d.name} — ${d.breed}${d.notes ? `: “${d.notes}”` : ""}`).join(””)}</div>`).join("")} <br><button onclick="window.print()">🖨️ Print</button></body></html>`);
win.document.close();
};
